<template>
  ...
  <div v-else>
    ...
    <nav
      class='nav nav-underline bg-soft-primary pb-0 text-center'
      ...
      <div class='dropdown m-3 w-100'>
        ...
        <ul class='dropdown-menu w-100' data-popper-placement='bottom-start'>
          <li v-for='item in enrollmentHistory' :key='item' :value='item.id'>
            <router-link
              @click='changeGrade(encrypt(item.id))'
              class='dropdown-item'
              :to="{
...
export default {
  ...
  mounted() {
    ...
    var link = 'student/semestral-grade'
  ...
  methods: {
    ...
    changeGrade(data) {
      ...
      this.subjectList = []
      var link = 'student/semestral-grade?key=' + data
      axios
        .get(link, {
          headers: {
            Authorization: 'Bearer ' + this.token
